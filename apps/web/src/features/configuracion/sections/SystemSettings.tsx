...
export default function SystemSettings({ handleReset, isDemoMode = false }: Props) {
  const [activeSubTab, setActiveSubTab] = useState<'logs' | 'reset'>('logs')
    ...
    <div className="space-y-8 animate-in fade-in slide-in-from-bottom-4 duration-500">
      ...
      <div className="flex items-center gap-2 border-b border-slate-200 pb-px">
        <button
          ...
          className={`flex items-center gap-2 px-6 py-3 text-[11px] font-bold uppercase tracking-widest transition-all border-b-2 ${
            activeSubTab === 'logs'
        ...
        <section className={`bg-white border rounded-3xl p-8 shadow-sm space-y-6 ${isDemoMode ? 'border-amber-200' : 'border-rose-200'}`}>
          ...
          <div className="p-6 border border-slate-200 rounded-3xl opacity-50 grayscale">
            <div className="flex items-center justify-between">
               <div className="max-w-md">
                  <p className="text-sm font-bold text-slate-900 italic">Exportar Base de Datos (SQL/CSV)</p>
                  <p className="text-[11px] text-slate-500 font-medium mt-1 leading-relaxed">
                    Genera un respaldo completo de la base de datos local para almacenamiento en frío.
                  </p>
               </div>
