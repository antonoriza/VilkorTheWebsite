...
test.describe("Vilkor Multi-language UI/UX Tests", () => {
  for (const lang of languages) {
    test.describe(`Language: ${lang.code.toUpperCase()}`, () => {
      ...
      test(`should show correct contact button text in ${lang.code}`, async ({
        page,
      }) => {
        await page.goto(lang.path);
      ...
      test(`should toggle pricing correctly in ${lang.code}`, async ({
        page,
      }) => {
        await page.goto(lang.path);
        ...
        await expect(starterPrice).toHaveText(
          `${lang.currency}${lang.monthly}`,
        );
        ...
        await expect(starterPrice).toHaveText(
          `${lang.currency}${lang.annual}`,
          { timeout: 5000 },
        );
      });

      test(`should show localized success message in ${lang.code}`, async ({
        page,
      }) => {
