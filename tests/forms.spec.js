...
test.describe('Vilkor Contact Form', () => {
  ...
  test('should show browser validation errors for empty fields', async ({ page }) => {
  ...
  test('should show success message on successful submission (mocked)', async ({ page }) => {
    ...
    const text = await page.evaluate(() => document.querySelector('#final-success-check-123').textContent);
    expect(text).toContain('Gracias');
  });

  test('should show error message on API failure (mocked)', async ({ page }) => {
